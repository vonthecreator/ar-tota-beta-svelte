...
<script lang="ts">
  ...
  const getTxData = async (gateway: string, txids: string[]) => {
    ...
    return await Promise.all(
      res.transactions.edges.map(async (edge: any) => {
        ...
        data.map(async (res: any) => {
          ...
          if (res.type && res.type === "application/x.arweave-manifest+json") {
            ...
            if (tracks.length > 0) {
              ...
              const tracks = data.map((res: any) => {
                const tags = res.tags;

                const id = res.id;
                const trackName = tags.find((tag: Tag) => tag.name === "Title")?.value;
                const artworkSrc =
                  thumbnail ||
                  tags.find((tag: Tag) => tag.name === "Thumbnail")?.value;
          ...
          const id = res.id;
          const trackName = tags.find((tag: Tag) => tag.name === "Title")?.value;
          const artworkSrc = tags.find((tag: Tag) => tag.name === "Thumbnail")?.value;
