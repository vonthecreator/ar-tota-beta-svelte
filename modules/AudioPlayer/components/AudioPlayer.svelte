...
<script lang="ts">
  ...
  import { KeyboardEvent, useEffect, useState } from "svelte";
  import {
    MdPause,
    MdPlayArrow,
    MdSkipNext,
  ...
  import { BsThreeDots } from "svelte-icons/bs";
  import { MdVolumeDown, MdVolumeUp } from "svelte-icons/md";
    ...
    return (
      ...
      >
        ...
        {!showContent && !showPaymentDialog && (
          <Button
            on:click={handleShowPaymentDialog}
            css={{
              m: "auto",
              position: "absolute",
              top: "65%",
        ...
        {#if showContent && !licenseIsPaidLoading}
          ...
          >
            <ProgressContainer>
              <ProgressSlider
                ...
                bind:value={scrubbing ? [scrubbedValue as number] : [currentTime]}
                max={duration}
                step={progressStep}
                aria-label="Track Progress"
                on:valuechange={(e) => handleProgressChange(e.detail)}
                on:valuecommit={(e) => handleProgressCommit(e.detail)}
              >
                <SliderTrack>
                  <SliderRange />
                </SliderTrack>
            ...
            <Flex gap="3">
              ...
              >
                <VolumeSlider
                  ...
                  step={progressStep}
                  aria-label="Volume"
                  on:valuechange={(e) => handleValueChange(e.detail)}
                  on:keydown={handleKeyDown}
                >
                  <SliderTrack>
                    <SliderRange />
